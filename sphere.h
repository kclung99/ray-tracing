...
public:
    ...
    bool hit(const ray &r, interval ray_t, hit_record &rec) const override
    {
        ...
        auto h = dot(r.direction(), oc); // simplify w/ b = -2h
        ...
        rec.t = root;
        rec.p = r.at(rec.t);
        vec3 outward_normal = (rec.p - current_center) / radius; // normalized
...
private:
    ...
    {
        // p: point on sphere of radius one centered at origin

        auto theta = std::acos(-p.y());            // vertical angle
        auto phi = std::atan2(-p.z(), p.x()) + pi; // horizontal angle
